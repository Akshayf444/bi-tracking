...
class Doctor_Model extends MY_model {
    ...
    public function getDoctor($VEEVA_Employee_ID = 0, $type) {
        ...
        $this->db->where(array('ed.Local_Employee_ID' => $VEEVA_Employee_ID, 'dm.Individual_Type' => $type));
    ...
    public function getProfilingDoctor($type) {
        ...
        $this->db->where(array('ed.Local_Employee_ID' => $this->VEEVA_Employee_ID, 'dm.Individual_Type' => $type, 'pf.Doctor_id' => NULL));
        $this->db->group_by("dm.Account_Name");
        $query = $this->db->get();
